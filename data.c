/****************************************data.c*****************************************
 ...
 Compile & run program
   -   make
   -  ./data -s <number> -t <number> -o    OR    ./data -s <number> -t <number>
**********************************************************************************************/
...
/* Run pixel calculations */
void* calculations(void* argument){
    ...
    int heightDivide = HEIGHT/threads;
    int minHeight = HEIGHT - (heightDivide * (threads - new->threadNum));
    int maxHeight = minHeight + heightDivide;
    ...
    for(y=minHeight;y<maxHeight;y++){
        for(x=0;x<WIDTH;x++){
            ...
            new->threadImg[(x + y*WIDTH)*3 + 0] = (unsigned char)min(red*255.0f, 255.0f);
            new->threadImg[(x + y*WIDTH)*3 + 1] = (unsigned char)min(green*255.0f, 255.0f);
            new->threadImg[(x + y*WIDTH)*3 + 2] = (unsigned char)min(blue*255.0f, 255.0f);
...
threadInfo *initializeThreadInfo(int threadNum, unsigned char *img){
    threadInfo *new;
    
    new = (threadInfo*)malloc(sizeof(threadInfo));
    
    new->threadNum = threadNum;
    new->threadImg = img;
...
int main(int argc, char *argv[]){
    ...
    threadInfo *threadSpec[threads+200];
    int check1 = 0;
    int check2 = 0;
    time_t begin, end;
    ...
    /* dynamically allocate thread handles */
    threadHandles = (pthread_t*)malloc(sizeof(pthread_t)*threads);
    ...
    for (int i = 0; i < threads; i++){
        threadSpec[i] = initializeThreadInfo(i, img);
        pthread_create(&threadHandles[i], NULL, calculations, (void *) &threadSpec[i]);
    }
