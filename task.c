/****************************************task.c*****************************************
 Student Name: Anugesh Balasubramaniam  Student Number: 0883563
 Date: 09/28/2018                    Course Name: CIS*3090
 ...
 Compile & run program
 -   make
 -  ./task -s <number> -t <number> -o    OR    ./task -s <number> -t <number>
 **********************************************************************************************/
...
threadInfo *initializeThreadInfo(int x, int y, float red, float green, float blue, unsigned char *img){
    threadInfo *new;
    
    new = (threadInfo*)malloc(sizeof(threadInfo));
...
/* process red for RGB */
void *processRED(void* argument){
    threadInfo *new = *((threadInfo **)argument);
    
    new->threadImg[(new->x + new->y * WIDTH)*3 + 0] = (unsigned char)min(new->red*255.0f, 255.0f);
    
    return NULL;
    
}

/* process green for RGB */
void *processGREEN(void* argument){
    threadInfo *new = *((threadInfo **)argument);
    
    new->threadImg[(new->x + new->y * WIDTH)*3 + 1] = (unsigned char)min(new->green*255.0f, 255.0f);
    
    return NULL;
}

/* process blue for RGB */
void *processBLUE(void* argument){
    threadInfo *new = *((threadInfo **)argument);
    
    new->threadImg[(new->x + new->y * WIDTH)*3 + 2] = (unsigned char)min(new->blue*255.0f, 255.0f);
...
int main(int argc, char *argv[]){
    ...
    pthread_t *threadHandles = (pthread_t*)malloc(sizeof(pthread_t)*3);
    threadInfo *threadSpec[threads+WIDTH];
    ...
    for(y=0;y<HEIGHT;y++){
        for(x=0;x<WIDTH;x++){
            ...
            /* create thread specifications and thread handles */
            threadSpec[x] = initializeThreadInfo(x,y,red,green,blue,img);
            pthread_create(&threadHandles[0], NULL, processRED, (void *) &threadSpec[x]);
            pthread_create(&threadHandles[1], NULL, processGREEN, (void *) &threadSpec[x]);
            pthread_create(&threadHandles[2], NULL, processBLUE, (void *) &threadSpec[x]);
            
            /* run thread handles */
            for (int thread_num = 0; thread_num < 3; thread_num++){
                pthread_join(threadHandles[thread_num], NULL);
            }
